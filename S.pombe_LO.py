...
for kfold in range(iteration): 
  ...
  I = np.eye(n) #单位矩阵
  alpha=0.00001  
  Z=X-np.dot(X,np.linalg.inv(I+alpha*np.dot(X,X)))  #X是邻接矩阵，Z是链接概率矩阵
  for i in range(1,n):
      for j in range(i):
         Z[i,j],X[i,j],X1[i,j],X2[i,j]=0,0,0,0 #左下角元素令为0，保留主对角线   
  np.savetxt('//extend2//chenyu//Sim//S.pombe//LO_fold%d.txt'%(kfold+1),Z, fmt='%s')   
  ...
  re1=P+N
  re1.sort(reverse=True) #将综合记录的LO得分,标签，以及节点对的名称按照得分从大到小排序
  re1=re1[:top]
  np.savetxt('//extend2//chenyu//Sim//S.pombe//LOscore_fold%d.txt'%(kfold+1),re1,fmt='%s') #保存LO的这些信息为txt文档 

############################################################################### 
  label1=np.array([re1[i][1] for i in range(len(re1))]) #标签
  precision1=np.divide(np.cumsum(label1>0), np.array(range(len(re1)))+1) 

  np.savetxt('//extend2//chenyu//Sim//S.pombe//LOprecision_fold%d.txt'%(kfold+1),precision1)
  mean_precision.append(precision1)
  print('LO Precision of Top100,Top500 and Top1000 are',precision1[99],precision1[499],precision1[999])
  t6=time.time()
  print("fold%d time:%s"%(kfold+1,t6-t2))
  print('-------------------------------------------------------------------')
################################################################### 
mean_P=np.mean(mean_precision,axis=0)
print('LO Mean precision of Top100,Top500 and Top1000 are',mean_P[99],mean_P[499],mean_P[999])
#打印正样本数目以及在所有样本中的比例
