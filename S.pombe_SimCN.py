...
############################################################################### 
  ...
  np.savetxt('//extend2//chenyu//Sim//S.pombe//SimCN_fold%d.txt'%(kfold+1),Z, fmt='%s')   
  ...
  re1=re1[:top]
  np.savetxt('//extend2//chenyu//Sim//S.pombe//SimCNscore_fold%d.txt'%(kfold+1),re1,fmt='%s') #保存Sim的这些信息为txt文档 

############################################################################### 
  label1=np.array([re1[i][1] for i in range(len(re1))]) #标签
  precision1=np.divide(np.cumsum(label1>0), np.array(range(len(re1)))+1) 

  np.savetxt('//extend2//chenyu//Sim//S.pombe//SimCNprecision_fold%d.txt'%(kfold+1),precision1)
  mean_precision.append(precision1)
  print('SimCN Precision of Top100,Top500 and Top1000 are',precision1[99],precision1[499],precision1[999])
  t6=time.time()
  print("fold%d time:%s"%(kfold+1,t6-t2))
################################################################### 
mean_P=np.mean(mean_precision,axis=0)
print('SimCN Mean precision of Top100,Top500 and Top1000 are',mean_P[99],mean_P[499],mean_P[999])
#打印正样本数目以及在所有样本中的比例
